...
from utils.utils_data import tokenize_and_align_labels
...
from utils.utils_train import TrainingArgsWithLossCoefs, get_trainer
...
def do_predict_eval(
    ...
    tagger = SequenceTagger(
        model, tokenizer, training_args=config.training, trainer=trainer, sngp_output=(config.ue.ue_type == "sngp")
    )
    eval_results = {}
...
def train_eval_conll2003_model(config, training_args, data_args, work_dir):
    ...
    if (
        ...
        or config.ue.ue_type in ['mc_maha', "maha", "nuq"]
    ):
    ...
    if config.do_eval:
        do_predict_eval(
            ...
            tokenizer,
            trainer,
            validation_dataset,#eval_dataset,#evaluate on validation
            train_dataset,#validation_dataset,#calibrate on train
            train_dataset,
            metric,
            config,
            data_args,
